...
const getReview = async (product_id, page, count, sort) => {
  ...
  //     break;
  //   default:
  //     sorter = 'helpfulness DESC, date DESC';
  //     break;
  // }
  let query1 = `select array_agg
  (json_build_object('review_id', review_id, 'rating', rating, 'summary', summary, 'recommended', recommended, 'response', response, 'body', body, 'date', (to_timestamp(date/1000)::date), 'reviewer_name', reviewer_name, 'helpfulness', helpfulness, 'photos', (select coalesce(array_agg(json_build_object('id', photo_id, 'url', url)), '{}')
  as photos from photos where reviews.review_id = photos.review_id)
  ))
  as results from reviews where product_id=$1 and reported=false`;
